...
export default class ApiService {
    ...
    index = async () => {
        return Promise.all([
            ...
            .then(response =>
                resolve({
                    mostPopular: response[0].data.items.slice(0, 10).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
    ...
    movieIndex = async () => {
        return Promise.all([
            ...
            .then(response =>
                resolve({
                    action: response[0].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                    adventure: response[1].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                    scifi: response[2].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                    animation: response[3].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                }),
