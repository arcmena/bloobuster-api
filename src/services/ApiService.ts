...
export default class ApiService {
    ...
    movieIndex = async () => {
        return Promise.all([
            ...
            .then(response =>
                resolve({
                    mostPopular: response[0].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                    comingSoon: response[1].data.items.map(movie => {
                    ...
                    adventure: response[3].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                    ...
                    animation: response[5].data.items.slice(0, 25).map(movie => {
                        const resizedPoster = movie.image.split('V1_')[0].concat('V1_UX192_CR0,4,192,264_AL_.jpg');
                        return { ...movie, resizedPoster };
                    }),
                }),
